...
class KeyEditWindow(Gtk.Window):
    ...
    Handles two key storage modes:

    File-path mode (signed_by is a /path/to/file.gpg):
      - "Key content" tab: shows and edits the key file content.
      - "Update" tab: replace from a new file or URL.

    Inline mode (signed_by is an embedded PGP block):
      - Path field is replaced with an info note.
      - "Key content" tab: shows and edits the inline PGP text; saves back inline.
      - "Update" tab: Fetch/Browse embed inline; Paste group for pasting a replacement.

    Emits "key-saved" with the resulting signed_by value (file path or inline content).
    """
    ...
    def __init__(self, repo: Repository, **kwargs) -> None:
        ...
        self._is_inline = bool(signed_by) and signed_by.lstrip().startswith("-----BEGIN PGP")
        self._file_existed = bool(signed_by) and not self._is_inline and Path(signed_by).exists()
    ...
    def _build_ui(self) -> None:
        ...
        # Key file location — info row for inline keys, editable path for file keys
        path_group = Adw.PreferencesGroup(title="Key file location")
        if self._is_inline:
            info_row = Adw.ActionRow(
                title="Storage",
                subtitle="Embedded inline in the .sources file — no separate key file",
            )
            path_group.add(info_row)
            self._path_row = None
        else:
            self._path_row = Adw.EntryRow(title="Key file path")
        ...
        # Notebook — switch-page connected after _error_lbl/_action_btn exist
        self._notebook = Gtk.Notebook()
        ...
        if not self._is_inline and not self._file_existed and signed_by:
        ...
        # Stack: spinner while loading key from disk, then the text view
        self._content_stack = Gtk.Stack()
        self._content_stack.add_css_class("card")
        ...
        spinner_box.append(Gtk.Spinner(spinning=True))
        spinner_box.append(Gtk.Label(label="Reading key…", css_classes=["dim-label"]))
        self._content_stack.add_named(spinner_box, "loading")
        ...
        self._content_stack.add_named(content_scroll, "content")

        # Always start with spinner for keys that need loading; content is set in _on_map
        self._content_stack.set_visible_child_name("loading" if (self._is_inline or self._file_existed) else "content")
        ...
        file_group = Adw.PreferencesGroup(
            title="Embed from file" if self._is_inline else "Replace from file",
            description="Read a key file and embed its content inline" if self._is_inline else None,
        )
        ...
        url_group = Adw.PreferencesGroup(
            title="Embed from URL" if self._is_inline else "Replace from URL",
            description="Fetch a key and embed its content inline" if self._is_inline else None,
        )
        ...
        if self._is_inline:
            paste_group = Adw.PreferencesGroup(title="Paste inline replacement")
            update_box.append(paste_group)
            paste_scroll = Gtk.ScrolledWindow(
                hscrollbar_policy=Gtk.PolicyType.NEVER,
                vscrollbar_policy=Gtk.PolicyType.AUTOMATIC,
                height_request=140,
            ...
            self._update_paste_buf = paste_view.get_buffer()
            self._update_paste_buf.connect("changed", lambda _: self._update_sensitivity())
            paste_scroll.set_child(paste_view)
            update_box.append(paste_scroll)
        ...
        self.set_child(outer)
        # All shared controls exist now — safe to connect switch-page
        self._notebook.connect("switch-page", self._on_tab_switched)
        # Connect after _action_btn exists; _on_map sets text, not _build_ui.
    ...
    def _on_tab_switched(self, _nb, _page, num: int) -> None:
        self._error_lbl.set_visible(False)
        if num == _EDIT_TAB_CONTENT:
            self._action_btn.set_label("Save changes")
        else:
            self._action_btn.set_label("Embed inline" if self._is_inline else "Replace key")
        self._update_sensitivity()
    ...
    def _update_sensitivity(self) -> None:
        ...
        if tab == _EDIT_TAB_CONTENT:
            ...
            current_path = "" if self._path_row is None else self._path_row.get_text().strip()
            ...
            if self._update_paste_buf is not None:
                t = self._update_paste_buf.get_text(
                    self._update_paste_buf.get_start_iter(),
                    self._update_paste_buf.get_end_iter(),
                    False,
                ).strip()
                paste_has_content = bool(t)
            enabled = (
                self._update_selected_file is not None or self._update_fetched_bytes is not None or paste_has_content
            )
    ...
    def _on_map(self, window: Gtk.Window) -> None:
        GLib.idle_add(lambda: window.set_focus(None) or GLib.SOURCE_REMOVE)
        if self._is_inline:
            # Content is already in memory; set it in an idle so the window
            # appears before the TextView layout runs.
            GLib.idle_add(self._on_key_loaded, self._original_signed_by.strip())
        elif self._file_existed:
            threading.Thread(
                target=self._load_key_content,
                args=(self._original_signed_by,),
                daemon=True,
            ).start()

    def _load_key_content(self, path: str) -> None:
        loaded = gpg.read_key_content(Path(path))
        GLib.idle_add(self._on_key_loaded, loaded)

    def _on_key_loaded(self, content: str | None) -> bool:
        if content:
            self._original_content = content
            self._content_view.get_buffer().set_text(content)
    ...
    def _on_update_fetch_done(self, key_bytes: bytes | None, err: str | None) -> bool:
        ...
        self._update_selected_file = None
        self._selected_file_lbl.set_visible(False)
        self._fetch_status_lbl.set_label(f"Key fetched — {len(key_bytes):,} bytes")
        self._fetch_status_lbl.remove_css_class("warning")
        self._fetch_status_lbl.add_css_class("success")
    ...
    def _do_action(self) -> None:
        ...
        if tab == _EDIT_TAB_CONTENT:
            ...
            if self._is_inline:
                # Embed the edited content back inline in the .sources file
                self._repo.signed_by = content
                writes.append({"path": str(self._repo.source_file), "content": repo_to_deb822(self._repo)})
                ...
                writes.append({"path": str(self._repo.source_file), "content": repo_to_deb822(self._repo)})
                result_value = key_path

        else:  # _EDIT_TAB_UPDATE
            if self._is_inline:
                result_value = self._do_action_update_inline(writes)
                if result_value is None:
                    return  # error already scheduled
            else:
                result_value = self._do_action_update_file(writes)
                if result_value is None:
                    return
        ...
        result = subprocess.run([PKEXEC, POLKIT_HELPER], input=payload, capture_output=True, text=True)
        if result.returncode == 0:
            GLib.idle_add(self._on_action_success, result_value)
        else:
            GLib.idle_add(self._on_action_failed, result.stderr.strip())

    def _do_action_update_inline(self, writes: list) -> str | None:
        """Resolve the update source for inline keys; embed content in signed_by. Returns result_value or None on error."""
        if self._update_fetched_bytes is not None:
            content = self._update_fetched_bytes.decode("ascii", errors="replace").strip()
        elif self._update_selected_file:
            content = gpg.read_key_content(Path(self._update_selected_file))
            if not content:
                GLib.idle_add(self._on_action_failed, "Could not read key content from the selected file.")
                return None
            content = content.strip()
        elif self._update_paste_buf is not None:
            content = self._update_paste_buf.get_text(
                self._update_paste_buf.get_start_iter(),
                self._update_paste_buf.get_end_iter(),
                False,
            ).strip()
            if not content:
                GLib.idle_add(self._on_action_failed, "No key content pasted.")
                return None
            valid, error = gpg.verify_key(content)
            if not valid:
                GLib.idle_add(self._on_action_failed, f"Key verification failed: {error}")
    ...
    def _do_action_update_file(self, writes: list) -> str | None:
        """Resolve the update source for file-path keys. Returns key_path or None on error."""
        key_path = self._path_row.get_text().strip()
        if self._update_fetched_bytes is not None:
            if not key_path:
                GLib.idle_add(self._on_action_failed, "Key file path is required.")
    ...
    def _on_action_success(self, result_value: str) -> bool:
        self.emit("key-saved", result_value)
        self.close()
        return GLib.SOURCE_REMOVE
